...
function getRates($rateId = null)
{
    ...
    if($rateId != null){
        $query = $db->prepare('SELECT pr.*, u.first_name, u.last_name, p.name FROM products_rates pr INNER JOIN users u ON u.id = pr.user_id INNER JOIN products p ON p.id = pr.product_id WHERE id = ?');
