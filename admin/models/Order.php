...
function getOrders($orderId = null)
{
    ...
    if($orderId != null){
        ...
        $query = $db->query('SELECT o.id, u.first_name, u.last_name, SUM(op.price * quantity) AS total, COUNT(op.id) AS nb_products FROM orders o INNER JOIN orders_products op ON o.id = op.order_id INNER JOIN users u ON u.id = o.user_id GROUP BY o.id ');
